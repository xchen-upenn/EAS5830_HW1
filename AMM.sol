...
contract AMM is AccessControl{
	...
	*/
    ...
    uint256 reserveIn = ERC20(inToken).balanceOf(address(this));
    uint256 reserveOut = ERC20(outToken).balanceOf(address(this));
    require(reserveIn > 0 && reserveOut > 0, "No liquidity");

    // import
    require( ERC20(inToken).transferFrom(msg.sender, address(this), sellAmount), "transferFrom failed" );
    ...
    require(outAmt > 0 && outAmt <= reserveOut, "Insufficient output");

    // Transfer out
    require( ERC20(outToken).transfer(msg.sender, outAmt), "transfer outToken failed" );

    emit Swap(inToken, outToken, sellAmount, outAmt);
